...
be used in Perbug Core with the `-asmap` option.
...
to binary before being used with Perbug Core.
...
the `perbug-cli getnodeaddresses` command.
...
```
perbug-cli getnodeaddresses 0 > addrs.json
```
