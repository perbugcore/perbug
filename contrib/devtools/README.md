...
Provides utilities for managing copyright headers of `The Perbug Core
...
Generates a perbug.conf file in `share/examples/` by parsing the output from `perbugd --help`. This script is run during the
release process to include a perbug.conf with the release binaries and can also be run by users to generate a file locally.
...
```bash
BUILDDIR=$PWD/my-build-dir contrib/devtools/gen-perbug-conf.sh
```
