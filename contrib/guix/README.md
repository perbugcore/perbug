# Bootstrappable Perbug Core Builds

This directory contains the files necessary to perform bootstrappable Perbug
Core builds.
...
To build Perbug Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Perbug Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/perbug-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/perbug-core/guix.sigs).
