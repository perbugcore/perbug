#!/usr/bin/env python3
# Copyright (c) 2020-2022 The Perbug Core developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing perbug-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='perbug-cli', help='perbug-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Perbug address to which the faucet should send')
...
parser.add_argument('perbug_cli_args', nargs='*', help='Arguments to pass on to perbug-cli (default: -signet)')

args = parser.parse_args()

if args.perbug_cli_args == []:
    args.perbug_cli_args = ['-signet']


def perbug_cli(rpc_command_and_params):
    argv = [args.cmd] + args.perbug_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = perbug_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = perbug_cli(['getnewaddress', 'faucet', 'bech32'])
