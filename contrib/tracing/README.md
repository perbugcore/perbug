...
Tracing (USDT) support for Perbug Core on Linux using. For more information on
USDT support in Perbug Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `perbugd` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `perbugd` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Perbug Core USDT documentation] on how to list available tracepoints in your
`perbugd` binary.

[Perbug Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py $(pidof perbugd)
```
...
Perbug P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py $(pidof perbugd)
```
...
In a different terminal, starting Perbug Core in SigNet mode and with
re-indexing enabled.

```
$ ./build/bin/perbugd -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py $(pidof perbugd)
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py $(pidof perbugd)
```
