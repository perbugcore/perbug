#!/usr/bin/env python3
# Copyright (c) 2022 The Perbug Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

""" Example logging Perbug Core mempool events using the mempool:added,
...
def main(pid):
    print(f"Hooking into perbugd with pid {pid}")
    perbugd_with_usdts = USDT(pid=int(pid))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    perbugd_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    perbugd_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    perbugd_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    perbugd_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[perbugd_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "<pid of perbugd>")
        exit(1)
