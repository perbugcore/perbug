...
"""Script for verifying Perbug Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from perbugcore.org and perbug.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/perbug-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://perbugcore.org"
HOST2 = "https://perbug.org"
VERSIONPREFIX = "perbug-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Perbug community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"perbug_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by perbugcore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the perbug release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes perbug.org lags behind perbugcore.org.)')
    )
