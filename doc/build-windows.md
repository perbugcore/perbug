...
The options known to work for building Perbug Core on Windows are:
...
Note that for WSL the Perbug Core source path MUST be somewhere in the default mount file system, for
example /usr/src/perbug, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
```shell
cmake --install build --prefix /mnt/c/workspace/perbug
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/perbug --strip
```
