# CJDNS support in Perbug Core

It is possible to run Perbug Core over CJDNS, an encrypted IPv6 network that
...
done outside of Perbug Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Perbug network and individual nodes.
...
before it will work with your Perbug Core node. This is described in steps
...
## Run Perbug Core with CJDNS

Once you are connected to the CJDNS network, the following Perbug Core
...
When enabled, this option tells Perbug Core that it is running in an
...
IPv6 local network. This helps Perbug Core perform better address management:
...
CJDNS support was added to Perbug Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `perbug-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/perbug/perbug/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Perbug Core

There are several ways to see your CJDNS address in Perbug Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `perbug-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `perbug-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `perbug-cli help getnodeaddresses` for details.
