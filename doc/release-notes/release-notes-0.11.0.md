Perbug Core version 0.11.0 is now available from:

  <https://perbug.org/bin/perbug-core-0.11.0/>
...
For example, add the following to `perbug.conf`:
...
related to the blockchain in the perbug system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Perbug Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Perbug Core will 
...
For an example script see [perbug-submittx](https://github.com/laanwj/perbug-submittx).
...
- #5334 `cf87536` libperbugconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new perbug-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only PerbugAmountField
- #5753 `0f44672` Add perbug logo to about screen
...
- #5809 `46bfbe7` Add perbug-cli man page
...
- #5849 `41259ca` contrib/init/perbugd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Perbug-Qt.app to Perbug-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for perbug-tx)
