...
Perbug Core version 0.20.0 is now available from:

  <https://perbugcore.org/bin/perbug-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from Perbug Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Perbug Core version 0.18.0.
...
- Testing or debugging of implementations of the Perbug P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Perbug Core. Perbug Core logs debug messages
...
- OpenSSL is no longer used by Perbug Core.  (#17265)

- BIP70 support has been fully removed from Perbug Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/perbug/perbug/issues/16599),
  [PR #16702](https://github.com/perbug/perbug/pull/16702), and the
  `perbugd help` for more information.  This option is experimental and
...
- All custom settings configured when Perbug Core starts are now
...
- The "Start Perbug Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://perbugcore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Perbug Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Perbug Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/perbug-core/HWI) for
  signing.  Future versions of Perbug Core should support a GUI option
...
- Perbug Core's automatically-generated source code documentation is
  now available at https://doxygen.perbugcore.org.  (#17596)
...
- The `perbug-cli` utility used with the `-getinfo` parameter now
...
- The `perbug-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `perbugstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_perbug-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_perbug` binary (hebasto)
...
- #18702 Fix ASLR for perbug-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate perbug schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest perbug.conf (sdaftuar)
...
- #18022 Fix appveyor `test_perbug` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (perbug-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone perbug-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Perbug Core files (hebasto)
...
- #17119 Fix broken perbug-cli examples (andrewtoth)
...
- #17503 Remove bitness from perbug-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.perbugcore.org (laanwj)
...
- #17648 Rename wallet-tool references to perbug-wallet (hel-o)
...
- #16115 On perbugd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from perbug-cli -getinfo (malevolent)
