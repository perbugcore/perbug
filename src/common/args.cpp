...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //perbug-tx using stdin
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Perbug
    //   new: C:\Users\Username\AppData\Local\Perbug
    // macOS: ~/Library/Application Support/Perbug
    // Unix-like: ~/.perbug
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Perbug";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Perbug";
#else
    fs::path pathRet;
