...
Connection::~Connection()
{
    ...
    // object, which can run arbitrary blocking perbug code so they have to run
    ...
    // functions run, so client objects are fully disconnected before perbug
    // code in the destructors are run. This way if the perbug code tries to
...
void EventLoop::addAsyncCleanup(std::function<void()> fn)
{
    ...
    //
    // In perbug core, running these callbacks in the right order is
