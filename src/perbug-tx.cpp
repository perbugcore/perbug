...
static void SetupPerbugTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " perbug-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The perbug-tx tool is used for creating and modifying perbug transactions.\n\n"
                "perbug-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded perbug transaction, or with \"-create [commands]\" to create a hex-encoded perbug transaction.\n"
                "\n"
                "Usage: perbug-tx [options] <hex-tx> [commands]\n"
                "or:    perbug-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded perbug transaction
