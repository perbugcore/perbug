// Copyright (c) 2025 The Perbug Core developers
...
Options:
  -m, --multiprocess     Run multiprocess binaries perbug-node, perbug-gui.
  -M, --monolithic       Run monolithic binaries perbugd, perbug-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'perbug-qt [ARGS]' or 'perbug-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'perbugd [ARGS]' or 'perbug-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'perbug-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'perbug-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'perbug-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_perbug [ARGS]'.
  chainstate [ARGS] Run perbug kernel chainstate util, equivalent to running 'perbug-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_perbug [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_perbug-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "perbug-gui" : "perbug-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "perbug-node" : "perbugd");
        } else if (cmd.command == "rpc") {
            args.emplace_back("perbug-cli");
            // Since "perbug rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("perbug-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("perbug-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_perbug");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("perbug-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_perbug");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_perbug-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("perbug-util");
        } else {
...
//! Execute the specified perbugd, perbug-qt or other command line in `args`
...
//!             be a relative path to a perbugd, perbug-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/perbug ...
...
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    // (https://github.com/perbug/perbug/pull/31375#discussion_r1861814807)
