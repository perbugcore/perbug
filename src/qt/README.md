This directory contains the source code for the Perbug Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Perbug UI.
...
- ClientModel is used by the main application `perbuggui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a perbug:... link from a browser).
...
* UI elements like PerbugAmountField, which inherit from QWidget.
* `perbugstrings.cpp`: automatically generated
* `perbugunits.(h/cpp)`: BTC / mBTC / etc. handling
* `callback.h`
...
**Note:** Do not change `local/perbug_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `perbug-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `perbug-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `build/bin`, and select `perbug-qt`
9. You're all set! Start developing, building, and debugging the Perbug Core GUI
