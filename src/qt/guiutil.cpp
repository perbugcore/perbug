...
#include <qt/perbugaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setValidator(new PerbugAddressEntryValidator(parent));
    widget->setCheckValidator(new PerbugAddressCheckValidator(parent));
}
...
bool parsePerbugURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no perbug: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!PerbugUnits::parse(PerbugUnit::BTC, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parsePerbugURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parsePerbugURI(uriInstance, out);
}

QString formatPerbugURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("perbug:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(PerbugUnits::format(PerbugUnit::BTC, info.amount, false, PerbugUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openPerbugConf()
{
    ...
    /* Open perbug.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Perbug (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for Perbug*.lnk
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / fs::u8path(strprintf("perbug-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a perbug.desktop file to the autostart directory:
