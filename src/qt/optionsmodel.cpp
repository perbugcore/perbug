// Copyright (c) 2011-present The Perbug Core developers
...
/** Call node.updateRwSetting() with Perbug 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Perbug 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/perbug/perbug/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because perbug 22.x will not try to read these.
...
//! Convert enabled/size values to perbug -prune setting.
...
//! Get pruning enabled value to show in GUI from perbug -prune setting.
...
//! Get pruning size value to show in GUI from perbug -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayPerbugUnit")) {
        settings.setValue("DisplayPerbugUnit", QVariant::fromValue(PerbugUnit::BTC));
    }
    QVariant unit = settings.value("DisplayPerbugUnit");
    if (unit.canConvert<PerbugUnit>()) {
        m_display_perbug_unit = unit.value<PerbugUnit>();
    } else {
        m_display_perbug_unit = PerbugUnit::BTC;
        settings.setValue("DisplayPerbugUnit", QVariant::fromValue(m_display_perbug_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if perbug.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_perbug_unit);
...
{
    if (new_unit.isNull() || new_unit.value<PerbugUnit>() == m_display_perbug_unit) return;
    m_display_perbug_unit = new_unit.value<PerbugUnit>();
    QSettings settings;
    settings.setValue("DisplayPerbugUnit", QVariant::fromValue(m_display_perbug_unit));
    Q_EMIT displayUnitChanged(m_display_perbug_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/perbug/perbug/pull/8273
    ...
    // (https://github.com/perbug-core/gui/issues/567).
