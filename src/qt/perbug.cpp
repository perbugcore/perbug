...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaType<PerbugUnit>("PerbugUnit");
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. perbug_de.qm (shortcut "de" needs to be defined in perbug.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. perbug_de_DE.qm (shortcut "de_DE" needs to be defined in perbug.qrc)
...
static const char* qt_argv = "perbug-qt";

PerbugApplication::PerbugApplication()
...
void PerbugApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the PerbugApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", PerbugGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void PerbugApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool PerbugApplication::createOptionsModel(bool resetSettings)
{
...
void PerbugApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new PerbugGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &PerbugGUI::quitRequested, this, &PerbugApplication::requestShutdown);
...
void PerbugApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void PerbugApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool PerbugApplication::baseInitialize()
{
    return node().baseInitialize();
}

void PerbugApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &PerbugApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &PerbugApplication::handleRunawayException);
    connect(this, &PerbugApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &PerbugApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void PerbugApplication::parameterSetup()
{
...
void PerbugApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void PerbugApplication::requestInitialize()
{
...
void PerbugApplication::requestShutdown()
{
...
void PerbugApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // perbug: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &PerbugGUI::handlePaymentRequest);
            connect(window, &PerbugGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void PerbugApplication::handleRunawayException(const QString &message)
{
...
void PerbugApplication::handleNonFatalException(const QString& message)
{
...
WId PerbugApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool PerbugApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", PerbugGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(perbug);
    Q_INIT_RESOURCE(perbug_locale);
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see perbug-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see perbug-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse perbug.conf, determine network, switch to network specific
    ...
    // perbug: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
