// Copyright (c) 2015-2022 The Perbug Core developers
                  ...
                  QMessageBox::StandardButton confirm_type = QMessageBox::Yes)
{
    ...
    entry->findChild<PerbugAmountField*>("payAmount")->setValue(amount);
...
void CompareBalance(WalletModel& walletModel, CAmount expected_balance, QLabel* balance_label_to_check)
{
    ...
    QString balanceComparison = PerbugUnits::formatWithUnit(unit, expected_balance, false, PerbugUnits::SeparatorStyle::ALWAYS);
...
//
//     QT_QPA_PLATFORM=xcb     build/bin/test_perbug-qt  # Linux
//     QT_QPA_PLATFORM=windows build/bin/test_perbug-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   build/bin/test_perbug-qt  # macOS
void TestGUI(interfaces::Node& node, const std::shared_ptr<CWallet>& wallet)
{
    ...
    // Amount input
    PerbugAmountField* amountInput = receiveCoinsDialog.findChild<PerbugAmountField*>("reqAmount");
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        qWarning() << "Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
                      "with 'QT_QPA_PLATFORM=cocoa test_perbug-qt' on mac, or else use a linux or windows build.";
        return;
    }
#endif
    TestGUI(m_node);
