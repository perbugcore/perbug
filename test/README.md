This directory contains integration tests that test perbugd and its
...
perbugd and perbug-qt by interacting with them through the RPC and P2P
interfaces.
...
Before tests can be run locally, Perbug Core must be built.  See the [building instructions](/doc#building) for help.
...
The P2P and RPC ports used by the perbugd nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another perbugd
process running on the system (perhaps from a previous test which hasn't successfully
killed all its perbugd nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other perbugd processes are running.

On linux, the test framework will warn if there is another
perbugd process running when the tests are started.

If there are zombie perbugd processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
perbugd processes running on the system, so should not be used if any non-test
perbugd processes are being run.**
...
sure perbugd processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and perbugd `debug.log`s will all be dumped
...
`test_framework.log` and perbugd `debug.log`s can be combined into a single
...
call methods that interact with the perbugd nodes-under-test.

If further introspection of the perbugd instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/perbugd.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/perbugd <pid>
```
