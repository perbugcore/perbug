#!/usr/bin/env python3
# Copyright (c) 2025-present The Perbug Core developers
...
without the parent process' PerbugTestFramework also failing.
"""
...
class FeatureFrameworkStartupFailures(PerbugTestFramework):
    ...
    def _verify_startup_failure(self, test, internal_args, expected_exception):
        ...
        try:
            ...
            print("Unexpected child process timeout!\n"
                  ...
                  "meaning dangling perbugd processes are to be expected.\n"
    ...
    def run_test(self):
        ...
        self.log.info("Verifying inability to connect to perbugd's RPC interface due to wrong port results in one exception containing at least one OSError.")
        self._verify_startup_failure(
            TestWrongRpcPortStartupFailure, [f"--internal_node_start_duration={node_start_duration}"],
            r"AssertionError: \[node 0\] Unable to connect to perbugd after \d+s \(ignored errors: {[^}]*'OSError \w+'?: \d+[^}]*}, latest: '[\w ]+'/\w+\([^)]+\)\)"
        )

        self.log.info("Verifying startup failure due to invalid arg results in only one exception.")
        self._verify_startup_failure(
            TestInitErrorStartupFailure, [],
            r"FailedToStartError: \[node 0\] perbugd exited with status 1 during initialization\. Error: Error parsing command line arguments: Invalid parameter -nonexistentarg"
        )
...
class TestWrongRpcPortStartupFailure(InternalTestMixin, PerbugTestFramework):
    def add_options(self, parser):
...
class TestInitErrorStartupFailure(InternalTestMixin, PerbugTestFramework):
    def set_test_params(self):
        self.num_nodes = 1
...
class TestStartStopStartupFailure(InternalTestMixin, PerbugTestFramework):
    def set_test_params(self):
        self.num_nodes = 1
...
class TestSuccess(InternalTestMixin, PerbugTestFramework):
    def set_test_params(self):
        self.num_nodes = 1
