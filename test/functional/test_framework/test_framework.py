#!/usr/bin/env python3
# Copyright (c) 2014-present The Perbug Core developers
...
TMPDIR_PREFIX = "perbug_func_test_"
...
class Binaries:
    """Helper class to provide information about perbug binaries
    ...
    def node_argv(self, **kwargs):
        "Return argv array that should be used to invoke perbugd"
        return self._argv("node", self.paths.perbugd, **kwargs)

    def rpc_argv(self):
        "Return argv array that should be used to invoke perbug-cli"
        # Add -nonamed because "perbug rpc" enables -named by default, but perbug-cli doesn't
        return self._argv("rpc", self.paths.perbugcli) + ["-nonamed"]

    def tx_argv(self):
        "Return argv array that should be used to invoke perbug-tx"
        return self._argv("tx", self.paths.perbugtx)

    def util_argv(self):
        "Return argv array that should be used to invoke perbug-util"
        return self._argv("util", self.paths.perbugutil)

    def wallet_argv(self):
        "Return argv array that should be used to invoke perbug-wallet"
        return self._argv("wallet", self.paths.perbugwallet)

    def chainstate_argv(self):
        "Return argv array that should be used to invoke perbug-chainstate"
        return self._argv("chainstate", self.paths.perbugchainstate)

    def _argv(self, command, bin_path, need_ipc=False):
        """Return argv array that should be used to invoke the command. It
        either uses the perbug wrapper executable (if BITCOIN_CMD is set or
        need_ipc is True), or the direct binary path (perbugd, etc). When
        bin_dir is set (by tests calling binaries from previous releases) it
        always uses the direct path."""
        if self.bin_dir is not None:
            return [os.path.join(self.bin_dir, os.path.basename(bin_path))]
        elif self.paths.perbug_cmd is not None or need_ipc:
            # If the current test needs IPC functionality, use the perbug
            # wrapper binary and append -m so it calls multiprocess binaries.
            perbug_cmd = self.paths.perbug_cmd or [self.paths.perbug_bin]
            return perbug_cmd + (["-m"] if need_ipc else []) + [command]
        else:
            return [bin_path]


class PerbugTestMetaClass(type):
    """Metaclass for PerbugTestFramework.

    Ensures that any attempt to register a subclass of `PerbugTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'PerbugTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("PerbugTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("PerbugTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class PerbugTestFramework(metaclass=PerbugTestMetaClass):
    """Base class for a perbug test script.

    Individual perbug test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use perbug-cli instead of RPC for all commands")
    ...
    def get_binary_paths(self):
        ...
        binaries = {
            "perbug": "BITCOIN_BIN",
            "perbugd": "BITCOIND",
            "perbug-cli": "BITCOINCLI",
            "perbug-util": "BITCOINUTIL",
            "perbug-tx": "BITCOINTX",
            "perbug-chainstate": "BITCOINCHAINSTATE",
            "perbug-wallet": "BITCOINWALLET",
        }
        # Set paths to perbug core binaries allowing overrides with environment
        # variables.
        ...
        # BITCOIN_CMD environment variable can be specified to invoke perbug
        # wrapper binary instead of other executables.
        paths.perbug_cmd = shlex.split(os.getenv("BITCOIN_CMD", "")) or None
        return paths
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("PerbugRPC")
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a perbugd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple perbugd test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the perbugd
    ...
    def _start_logging(self):
        ...
        # Format logs the same as perbugd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in perbug.conf
    ...
    def skip_if_no_perbugd_tracepoints(self):
        """Skip the running test if perbugd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("perbugd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_perbugd_zmq(self):
        """Skip the running test if perbugd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("perbugd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if perbug-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("perbug-wallet has not been compiled")

    def skip_if_no_perbug_tx(self):
        """Skip the running test if perbug-tx has not been compiled."""
        if not self.is_perbug_tx_compiled():
            raise SkipTest("perbug-tx has not been compiled")

    def skip_if_no_perbug_util(self):
        """Skip the running test if perbug-util has not been compiled."""
        if not self.is_perbug_util_compiled():
            raise SkipTest("perbug-util has not been compiled")

    def skip_if_no_perbug_chainstate(self):
        """Skip the running test if perbug-chainstate has not been compiled."""
        if not self.is_perbug_chainstate_compiled():
            raise SkipTest("perbug-chainstate has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if perbug-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("perbug-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether perbug-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether perbug-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_perbug_tx_compiled(self):
        """Checks whether perbug-tx was compiled."""
        return self.config["components"].getboolean("BUILD_BITCOIN_TX")

    def is_perbug_util_compiled(self):
        """Checks whether perbug-util was compiled."""
        return self.config["components"].getboolean("ENABLE_BITCOIN_UTIL")

    def is_perbug_chainstate_compiled(self):
        """Checks whether perbug-chainstate was compiled."""
