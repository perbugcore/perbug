...
class ToolPerbugTest(PerbugTestFramework):
    def set_test_params(self):
        self.setup_clean_chain = True
        self.num_nodes = 1

    def skip_test_if_missing_module(self):
        # Skip test on windows because currently when `perbug node -version` is
        # run on windows, python doesn't capture output from the child
        # `perbugd` and `perbug-node` process started with _wexecvp, and
        # stdout/stderr are always empty. See
        # https://github.com/perbug/perbug/pull/33229#issuecomment-3265524908
    ...
    def set_cmd_args(self, node, args):
        """Set up node so it will be started through perbug wrapper command with specified arguments."""
        node.args = [self.binary_paths.perbug_bin] + args + ["node"] + self.node_options[node.index]
    ...
    def run_test(self):
        ...
        self.log.info("Ensure perbug node command invokes perbugd by default")
        self.test_args([], [], expect_exe="perbugd")

        self.log.info("Ensure perbug -M invokes perbugd")
        self.test_args(["-M"], [], expect_exe="perbugd")

        self.log.info("Ensure perbug -M does not accept -ipcbind")
        ...
        if self.is_ipc_compiled():
            self.log.info("Ensure perbug -m invokes perbug-node")
            self.test_args(["-m"], [], expect_exe="perbug-node")

            self.log.info("Ensure perbug -m does accept -ipcbind")
            self.test_args(["-m"], ["-ipcbind=unix"], expect_exe="perbug-node")

            self.log.info("Ensure perbug accepts -ipcbind by default")
            self.test_args([], ["-ipcbind=unix"], expect_exe="perbug-node")

            self.log.info("Ensure perbug recognizes -ipcbind in config file")
            append_config(node.datadir_path, ["ipcbind=unix"])
            self.test_args([], [], expect_exe="perbug-node")
